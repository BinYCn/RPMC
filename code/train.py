...
parser.add_argument('--exp', type=str,
                    default='RPMC', help='experiment_name')
...
parser.add_argument('--beta_o', type=float,
                    default=0.7, help='consistency_rampup')
parser.add_argument('--beta_t', type=float,
                    default=1.0, help='consistency_rampup')
parser.add_argument('--unc', type=float,
                    default=0.25, help='consistency_rampup')
...
def train(args, snapshot_path):
    ...
    ce_loss_1 = CrossEntropyLoss(reduction='none')
    mask_dice_loss = losses.mask_DiceLoss_1()
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            l_image = volume_batch[:args.labeled_bs]
            l_label = label_batch[:args.labeled_bs]

            X = list(zip(l_image, l_label))
            U = volume_batch[args.labeled_bs:]
            threshold = args.beta_o + (args.beta_t - args.beta_o) * ramps.sigmoid_rampup((iter_num // 150),
                                                                                         args.consistency_rampup)

            X_prime, U_prime, pseudo_label, pseudo_union = RPM(X, U, eval_net=ema_model, K=2,
                                                               T=args.temperature,
                                                               alpha=0.75, mixup_mode='_x',
                                                               aug_factor=1, beta=threshold, unc=args.unc)

            model.train()
            X_data = torch.stack([x[0] for x in X_prime], dim=0)
            X_label = torch.stack([x[1] for x in X_prime], dim=0)
            M_data = torch.stack([x[0] for x in U_prime], dim=0)
            M_label = torch.stack([x[1] for x in U_prime], dim=0)
            loss_mask = torch.stack([x[2] for x in U_prime], dim=0)

            X = torch.cat((X_data, M_data), 0)
            X_label = torch.cat((X_label, M_label), 0)
            ...
            loss_seg_ce_lab += ce_loss(y[:args.labeled_bs], X_label[:args.labeled_bs].long())
            loss_seg_dice_lab += dice_loss(y_prob[:args.labeled_bs], X_label[:args.labeled_bs].unsqueeze(1))
            loss_seg_ce_unlab += torch.mean(ce_loss_1(y[args.labeled_bs:], X_label[args.labeled_bs:].long()) * loss_mask)
            loss_seg_dice_unlab += mask_dice_loss(y_prob[args.labeled_bs:], X_label[args.labeled_bs:].unsqueeze(1), mask=loss_mask)
            ...
            writer.add_scalar('info/consist_loss',
                              loss_consist, iter_num)
            writer.add_scalar('info/consistency_weight',
                              consistency_weight, iter_num)
            logging.info(
                'iteration %d : loss : %f, supervised_loss: %f, pseudo_loss: %f, consist_loss: %f' %
                (iter_num, loss.item(), supervised_loss.item(), pseudo_loss.item(), loss_consist.item()))

            if iter_num >= 8000 and iter_num % 200 == 0:
                model.eval()
                if args.dataset_name == "LA":
                    ...
                    dice_sample = test_patch.var_all_case(model, num_classes=num_classes, patch_size=patch_size,
                                                          stride_xy=64, stride_z=64, dataset_name='BraTS2019')
                if dice_sample > best_performance:
                    best_performance = dice_sample
                    save_mode_path = os.path.join(snapshot_path,
                                                  'iter_{}_dice_{}.pth'.format(iter_num, best_performance))
